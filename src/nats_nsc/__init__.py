...
class Account(Auth):
    ...
    def sub_permissions(self) -> Permissions:
        ...
        return Permissions(
            allow=ps_dct["allow"] if "allow" in ps_dct else [], deny=ps_dct["deny"] if "deny" in ps_dct else []
        )
...
class User(Auth):
    ...
    def bearer(self) -> bool:
        ...
        return (
            self._jwt_payload["nats"]["bearer_token"] if "bearer_token" in self._jwt_payload["bearer_token"] else False
        )
